...
def get_path_to_tailPath(snake_list, obstacles, real_snake):
    ...
    while queue:
        ...
        for dv in dvs:
            newnode = [node[0] + dv[0], node[1] + dv[1]]
            if newnode not in obstacles and newnode not in snake_list and newnode not in visited and not hit_boundaries(
                    newnode):
                queue.append([node[0] + dv[0], node[1] + dv[1]])
                visited.append([node[0] + dv[0], node[1] + dv[1]])
                paths[tuple(newnode)] = node
...
def draw_snake(snake):
    for pos in snake:
        if pos == snake[0]:
            ...
            pygame.draw.rect(dis, green, [pos[0], pos[1], snake_block - 1, snake_block - 1])
        else:
            pygame.draw.rect(dis, blue, [pos[0], pos[1], snake_block - 1, snake_block - 1])
...
def draw_score(score):
    ...
    message(str, white, [30, 30])


def draw_level(level):
    str = "Level: " + level.__str__()
    message(str, white, [dis_width - 150, 30])
...
def button_rect():
    ...
    pygame.draw.rect(dis, (white), button_1)
    pygame.draw.rect(dis, (white), button_2)
...
def menuLoop():
    ...
    while not exit_game:
        ...
        message("WELCOME", white, [310, 200])
        mx, my = pygame.mouse.get_pos()
        button_1 = pygame.Rect(300, 300, 200, 50)
        button_2 = pygame.Rect(300, 400, 200, 50)
        button_rect()
        message("Play", greend, [365, 307])
        message("Quit", redd, [365, 407])
        if button_1.collidepoint((mx, my)):
            if click:
                mixer.music.load("res/playing.mp3")
                mixer.music.play()
                goto_gameplay = True
...
def gameLoop(level):
    ...
    while not game_over:
        ...
        while goto_nextlevel:
            dis.fill(dark_gray)
            message("Next level!", green, [dis_width / 3 + 60, dis_width / 5])
            message("Press N to go to the next level!", green, [dis_width / 3 - 50, dis_width / 5 + 100])
            pygame.display.update()
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_c:
                        mixer.music.load("res/ending.mp3")
                        mixer.music.play()
                        gameLoop(0)
        ...
        pygame.draw.rect(dis, yellow, rect)
...
mixer.init()
mixer.music.load("res/opening.mp3")
mixer.music.play()
menuLoop()
